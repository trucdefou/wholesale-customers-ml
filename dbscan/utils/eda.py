...
def graph_histogram( # Grafica histogramas
    ...
    figsize=(14, 10),
    title="Histogramas"):
    row_number = int(np.ceil(len(columns_df) / columns_number))  # Redondear para obtener el número de filas

    # Crear subplots
    fig, axes = plt.subplots(nrows=row_number, ncols=columns_number, figsize=figsize)

    # Asegurar que 'axes' sea siempre un arreglo plano
    axes = np.atleast_1d(axes).flatten()

    for idx, column in enumerate(columns_df):
    ...
    for idx in range(len(columns_df), len(axes)):
